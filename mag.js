/* ═══════════════════════════════════════════════════════
   ...
   v1.1: automatische Berechnung, Systemdruck, SV-Prüfung
═══════════════════════════════════════════════════════ */
'use strict';
...
function calcMAG() {
  ...
  // Praxiswert: Anlagen-/Fülldruck aus geodätischer Höhe
  // p_sys = h[m] × 0,1 + 0,5 bar
  ...
  // Vordruck etwas unter Fülldruck / Systemdruck
  const p0Auto = Math.max(.8, h * 0.1 + .3);
  const p0 = (!isNaN(p0Manual) && p0Manual > 0) ? p0Manual : p0Auto;

  const pFill = pSys;

  // Enddruck automatisch mit Reserve unter Sicherheitsventil
  const peAuto = Math.max(p0 + .5, pSV - .5);
  const pe = (!isNaN(peManual) && peManual > 0) ? peManual : peAuto;
  ...
  if (!pressureSpreadOk) {
    warnings.push('Enddruck muss deutlich über dem Vordruck liegen. Sicherheitsventil / Druckhaltung prüfen.');
  }

  if (pe >= pSV) {
    warnings.push('Enddruck liegt am oder über dem Sicherheitsventil-Ansprechdruck. Enddruck reduzieren oder Sicherheitsventil prüfen.');
  } else if (pe > pSV - 0.2) {
    warnings.push('Enddruck liegt sehr nah am Sicherheitsventil-Ansprechdruck. Reserve prüfen.');
  }

  warnings.push('Systemdruck aus Höhe: p = h × 0,1 + 0,5 bar.');

  if (system === 'solar') {
    warnings.push('Solar-MAG: Stillstandsvolumen und höhere Temperaturbelastung zusätzlich objektspezifisch prüfen.');
  }

  if (medium !== 'water') {
    warnings.push('Glykolgemisch berücksichtigt über Zuschlagsfaktor. Herstellerdaten bleiben maßgebend.');
  }
  ...
  const result = {
    system, medium, VA, tMin, tMax, h, pSV, pSys,
    e, VE, reserve, p0, pFill, pe, VNmin, recommended: rec
  };
...
function initMAG() {
  ...
  // Button entfällt: Berechnung läuft automatisch.
  calcMAG();
}
