...
const base = {
  ...
  refrigerantId: 'R32',
  chargeKg: '8',
  placedOnMarketDate: '31.12.2026',
  commissioningDate: '02.01.2027',
  stockAssessmentDate: '01.02.2028',
...
const incompleteState = { ...base, commissioningDate: '', stockAssessmentDate: '' };
const incompleteModel = buildFGasesResultModel(incompleteState, calculate(incompleteState));
...
const r513aState = { ...base, refrigerantId: 'R-513A', stockAssessmentDate: '01.02.2032' };
const r513a = calculate(r513aState);
assert.equal(r513a.serviceDetails.status, 'no-prohibition-found');
const r513aModel = buildFGasesResultModel(r513aState, r513a);
const serviceRows = r513aModel.groups.find(group => group.title === 'Wartung und Instandhaltung').rows;
assert.ok(serviceRows.every(row => !row.label.includes('Beschaffung neues HFKW')));
assert.equal(serviceRows.find(row => row.label === 'Servicekältemittel: neu').value, 'Verwendung möglich · HFKW-Quote begrenzt');
assert.equal(serviceRows.find(row => row.label === 'Rechtsgrundlage Serviceverbot').value, 'Art. 13 der Verordnung (EU) 2024/573');
...
const excessiveState = { ...base, specificRefrigerantLossPercent: '5' };
const excessive = calculate(excessiveState);
assert.equal(excessive.checks.operatorDuties, 'non-compliant');
const operatorRows = buildFGasesResultModel(excessiveState, excessive).groups.find(group => group.title === 'Betreiberpflichten').rows;
assert.ok(operatorRows.some(row => row.label === 'Eingetragener spezifischer Kältemittelverlust' && row.value === '5'));
assert.ok(operatorRows.some(row => row.label === 'Bewertung Kältemittelverlust' && row.value.includes('Grenzwert überschritten')));
...
console.log('F-Gases temporal, service and loss-limit regression passed.');
