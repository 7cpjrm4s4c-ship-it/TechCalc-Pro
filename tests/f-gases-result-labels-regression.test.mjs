...
assert.equal(model.primary.rows.find(row => row.label === 'Wartung / Instandhaltung').value, 'Verwendung möglich');
const serviceRows = model.groups.find(group => group.title === 'Wartung und Instandhaltung').rows;
assert.equal(serviceRows.find(row => row.label === 'Gesamtbewertung').value, 'Verwendung möglich');
assert.equal(serviceRows.find(row => row.label === 'Servicekältemittel: neu').value, 'Verwendung möglich');
