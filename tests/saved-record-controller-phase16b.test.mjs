...
assert.match(pipe + pipeController, /createSavedRecordActions|bindSavedRecordWorkflow|createLineSectionController/, 'pipe-sizing must use central saved-record workflow/actions');
assert.doesNotMatch(pipe + pipeController, /querySelector\('\[data-pipe-save\]'\)\?\.addEventListener/, 'pipe-sizing must not keep module-owned save listener');
