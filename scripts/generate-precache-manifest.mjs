...
function packageMeta() {
  ...
  if (!pkg.name || !pkg.version) throw new Error('package.json must define name and version for service-worker cache versioning');
  return pkg;
}
...
function renderServiceWorker(source, assets, cacheName, cacheRevision) {
  ...
  if (nextSource === source && !source.includes(renderCacheName(cacheName))) throw new Error('Could not replace const CACHE_NAME in service-worker.js');
  nextSource = nextSource.replace(/const CACHE_REVISION = '[^']+';/, renderCacheRevision(cacheRevision));
  if (!nextSource.includes(renderCacheRevision(cacheRevision))) throw new Error('Could not replace const CACHE_REVISION in service-worker.js');
  const withAssets = nextSource.replace(/const ASSETS = \[[\s\S]*?\];/, renderAssets(assets));
  if (withAssets === nextSource && !nextSource.includes(renderAssets(assets))) throw new Error('Could not replace const ASSETS block in service-worker.js');
  return withAssets;
}
