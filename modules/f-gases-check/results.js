import { formatRefrigerantLabel } from '../../core/data/index.js';
...
const STATUS_LABELS = Object.freeze({
  ...
  applies: 'Grenzwertprüfung erforderlich'
});

const INPUT_FIELD_LABELS = Object.freeze({
  placedOnMarketDate: 'Erstmaliges Inverkehrbringen',
  commissioningDate: 'Erstmalige Inbetriebnahme',
  stockAssessmentDate: 'Prüfdatum der Bestandsanlage',
  ...
  hermeticallySealedStatus: 'Hermetisch geschlossene Einrichtung nach Art. 3 Nr. 9 VO (EU) 2024/573',
  hermeticallySealedLabelStatus: 'Als hermetisch geschlossene Einrichtung gekennzeichnet',
...
function legalSourceLabel(source = '') {
  ...
  match = value.match(/^EU-FGAS:AnnexIV\((\d+)\)(?:\(([a-z])\))?$/i);
  if (match) return `Verordnung (EU) 2024/573, Anhang IV Nr. ${match[1]}${match[2] ? ` Buchst. ${match[2].toLowerCase()}` : ''}`;
...
function serviceOverallLabel(details = {}) {
  if (details.status === 'prohibited') return 'Serviceverbot vorhanden';
  if (details.status === 'allowed-under-exception') return 'Service nur unter gesetzlicher Ausnahme möglich';
  if (details.status === 'no-prohibition-found') return 'Keine Einschränkungen';
  if (details.status === 'incomplete') return 'Bewertung unvollständig';
  return formatFGasesStatus(details.status);
}

function serviceLabel(details = {}) {
  if (details.status === 'prohibited') return 'Verwendung nicht möglich';
  if (details.status === 'allowed-under-exception') return 'Verwendung eingeschränkt möglich';
  if (details.status === 'no-prohibition-found') return 'Verwendung möglich';
  if (details.status === 'incomplete') return 'Bewertung unvollständig';
  return formatFGasesStatus(details.status);
}

function serviceScenarioLabel(details = {}) {
  const base = serviceLabel(details);
  if (details.refrigerantOrigin !== 'new' || details.status !== 'no-prohibition-found' || !details.marketAvailability) return base;
  if (details.marketAvailability.status === 'quota-zero') return 'Verwendung eingeschränkt möglich';
  if (details.marketAvailability.status === 'quota-limited') return 'Verwendung möglich · HFKW-Quote begrenzt';
  return base;
}
...
function serviceScenarioRows(details = {}) {
  return (details.originScenarios || []).map(item => ({
    ...
    value: serviceScenarioLabel(item)
  }));
}

function hasHfcQuota(details = {}) {
  return Boolean(details.marketAvailability || (details.originScenarios || []).some(item => item.marketAvailability));
}

function obligationRows(items = [], state = {}) {
  return items.flatMap(item => {
    ...
    if (item.type === 'specific-refrigerant-loss') {
      if (item.status === 'exception-applies' && item.exceptionReason === 'hermetically-sealed-and-labelled') {
        return [
          { label: 'Maximal zulässiger spezifischer Kältemittelverlust', value: '§ 2 Abs. 1 nicht anwendbar' },
          { label: 'Begründung', value: 'Hermetisch geschlossene und entsprechend gekennzeichnete Einrichtung – Ausnahme nach § 2 Abs. 3 ChemKlimaschutzV' },
          { label: 'Rechtsgrundlage Kältemittelverlust', value: '§ 2 Abs. 3 ChemKlimaschutzV' }
        ];
      }
      const actual = Number(String(state.specificRefrigerantLossPercent ?? '').replace(',', '.'));
      const hasActual = hasValue(state.specificRefrigerantLossPercent) && Number.isFinite(actual);
      const exceeded = item.maximumPercent != null && hasActual && actual > item.maximumPercent;
      const assessment = exceeded
        ? 'Grenzwert überschritten – Betreiberpflicht nach § 2 Abs. 1 ChemKlimaschutzV nicht erfüllt'
        : item.maximumPercent != null && hasActual
          ? 'Grenzwert eingehalten'
          : item.maximumPercent != null
            ? 'Bewertung offen – spezifischer Kältemittelverlust nicht angegeben'
            : formatFGasesStatus(item.status);
      return [
        { label: 'Maximal zulässiger spezifischer Kältemittelverlust', value: item.maximumPercent == null ? formatFGasesStatus(item.status) : fmt(item.maximumPercent), unit: item.maximumPercent == null ? '' : '%' },
        { label: 'Eingetragener spezifischer Kältemittelverlust', value: hasActual ? fmt(actual) : '—', unit: hasActual ? '%' : '' },
        { label: 'Bewertung Kältemittelverlust', value: assessment },
        { label: 'Rechtsgrundlage Kältemittelverlust', value: '§ 2 Abs. 1 ChemKlimaschutzV' }
      ];
    }
...
function collectInputIssues(state = {}, result = {}) {
  ...
  const required = [
    ...
    ...(state.plannedActivity && state.plannedActivity !== 'installation' ? ['stockAssessmentDate'] : [])
  ];
  ...
  const inspectUnresolved = (entries, dateField) => {
    for (const entry of (entries || []).filter(item => item.status === 'unresolved')) {
      for (const rawReason of entry.reasons || []) {
        const field = rawReason === 'assessment-date'
          ? dateField
          : rawReason === 'installedAtSiteDate'
            ? 'commissioningDate'
            : rawReason;
  ...
  if (result.checks?.certification === 'incomplete') {
    const matched = new Set((result.lifecycleRegulationEvaluation || [])
      .filter(entry => ['matched', 'matched-with-unresolved-exception'].includes(entry.status))
...
export function buildFGasesResultModel(state = {}, result = {}) {
  ...
  const allEvaluations = [
    ...(result.placingOnMarketRegulationEvaluation || []),
    ...(result.regulationEvaluation || []),
    ...(result.lifecycleRegulationEvaluation || [])
  ];
  const manualRules = [...new Set(allEvaluations
    .filter(entry => entry.status === 'manual-review')
    .map(entry => entry.rule?.legalSource || entry.rule?.id)
    .filter(Boolean))];
  const inputIssues = collectInputIssues(state, result);
  const notices = [];
  if (manualRules.length) notices.push({
    title: 'Manuelle Rechtsprüfung',
    messages: [`Nicht automatisch entscheidbar: ${manualRules.map(legalSourceLabel).join(', ')}`],
    prefix: 'Hinweis'
  });
  ...
  const serviceRows = [
    { label: 'Gesamtbewertung', value: serviceOverallLabel(service) },
    ...serviceScenarioRows(service),
    { label: 'Rechtsgrundlage Serviceverbot', value: 'Art. 13 der Verordnung (EU) 2024/573' }
  ];
  if (hasHfcQuota(service)) serviceRows.push({ label: 'Rechtsgrundlage HFKW-Neuware', value: 'Art. 17 in Verbindung mit Anhang VII der Verordnung (EU) 2024/573' });
  const documentationRows = obligationRows(docs.obligations || [], state);
  const operatorDutyRows = obligationRows(duties.obligations || [], state);
  return {
    primary: {
      ...
      rows: [
        { label: 'Wartung / Instandhaltung', value: serviceOverallLabel(service) },
    ...
    groups: [
      {
        ...
        rows: [
          ...
          { label: 'Aktuell ausgewählte Tätigkeit', value: state.plannedActivity || 'installation' }
        ]
      },
      {
        title: 'Kältemittel und Klimawirkung',
        rows: [
          { label: 'Kältemittel', value: formatRefrigerantLabel(result.refrigerant) || state.refrigerantId || '—' },
      ...
      { title: 'Wartung und Instandhaltung', rows: serviceRows },
      {
      ...
      },
      {
        ...
        rows: documentationRows.length ? documentationRows : [{ label: 'Status', value: formatFGasesStatus(docs.status) }]
      },
      {
        title: 'Betreiberpflichten',
        rows: operatorDutyRows.length ? operatorDutyRows : [{ label: 'Status', value: formatFGasesStatus(duties.status) }]
      }
    ],
    notices
  };
