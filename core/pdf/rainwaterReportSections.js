...
const modeLabel=x=>x==='roof'?'Dachfläche':x==='property'?'Grundstücksfläche':x||'—';

function surfaceSection(s,i){
 const d=s.drain||{}, st=s.stack||{}, c=s.collector||{}, e=s.emergency||{};
 const rows=[
  r('Bezeichnung',s.name),r('Bereich',modeLabel(s.mode)),r('Flächenart',s.areaTypeLabel||s.areaType),
 ...
 rows.push(
  ...
  n('Anzahl Fallleitungen',st.count,'integer','Stk.'),n('Abfluss je Fallleitung',st.flowPerStackLs,'flow','l/s'),
 ...
 if(s.mode==='roof') rows.push(
  r('Art der Notentwässerung',emergencyLabel(e.type)),n('Basis-Notabfluss',e.baseFlowLs,'flow','l/s'),
  n('Sicherheitsfaktor',e.safetyFactor,'factor'),n('Bemessungsabfluss QNot',e.requiredFlowLs,'flow','l/s'),
  n('Anstauhöhe am Notüberlauf',e.headMm,'length','mm'),
  ...(e.type==='rect'?[n('Breite des Notüberlaufs',e.widthMm,'length','mm')]:[]),
  ...(e.type==='round'?[n('Durchmesser des Notüberlaufs',e.diameterMm,'length','mm')]:[]),
  ...(e.type==='manual'?[r('Hersteller-DN des Notüberlaufs',e.manufacturerDn)]:[]),
  n('Abflussvermögen je Notüberlauf',e.capacityLs,'flow','l/s'),
  n('Erforderliche Anzahl Notüberläufe',e.requiredCount,'integer','Stk.'),
  n('Rechnerisch erforderliche Gesamtbreite',e.requiredWidthMm,'length','mm')
 );
...
export function buildRainwaterReportSections(dto={}){
 ...
 if(a(dto.warnings).length) sections.push({
   title:`${sections.length+1}. Hinweise / Prüfhinweise`,
   rows:a(dto.warnings).map(x=>['',t(x),'']),
   isLineSection:false,
   forcePageBreak:true,
   fullWidthRows:true
 });
