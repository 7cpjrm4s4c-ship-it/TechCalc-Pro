...
const steelDimensions = [
  {dn:15, dimension:'21,3  x  2,65', di:16},
  {dn:20, dimension:'26,9  x  2,65', di:21.6},
  {dn:25, dimension:'33,7  x  3,25', di:27.2},
  {dn:32, dimension:'42,4  x  3,25', di:35.9},
  {dn:40, dimension:'48,3  x  3,25', di:41.8},
  {dn:50, dimension:'60,3  x  3,65', di:53},
  {dn:65, dimension:'76,1  x  2,9', di:70.3},
  {dn:80, dimension:'88,9  x  3,2', di:82.5},
  {dn:100, dimension:'114,3  x  3,6', di:107.1},
  {dn:125, dimension:'139,7  x  4,0', di:131.7},
  {dn:150, dimension:'168,3  x  4,5', di:159.3},
  {dn:200, dimension:'219,1  x  5,9', di:207.3},
  {dn:250, dimension:'273,0  x  6,3', di:260.4},
  {dn:300, dimension:'323,9  x  7,1', di:309.7}
];

const mapressDimensions = [
  {dn:10, dimension:'12  x  1,0', di:10},
  {dn:12, dimension:'15  x  1,0', di:13},
  {dn:15, dimension:'18  x  1,0', di:16},
  {dn:20, dimension:'22  x  1,2', di:19.6},
  {dn:25, dimension:'28  x  1,2', di:25.6},
  {dn:32, dimension:'35  x  1,5', di:32},
  {dn:40, dimension:'42  x  1,5', di:39},
  {dn:50, dimension:'54  x  1,5', di:51},
  {dn:65, dimension:'76,1  x  2,0', di:72.1},
  {dn:80, dimension:'88,9  x  2,0', di:84.9},
  {dn:100, dimension:'108  x  2,0', di:104}
];
...
const copperDimensions = [
  ...
  {dn:12, dimension:'15  x  1,0', di:13},
  {dn:15, dimension:'18  x  1,0', di:16},
  {dn:20, dimension:'22  x  1,0', di:20},
  {dn:25, dimension:'28  x  1,5', di:25},
  {dn:32, dimension:'35  x  1,5', di:32},
  {dn:40, dimension:'42  x  1,5', di:39},
  {dn:50, dimension:'54  x  2,0', di:50},
  {dn:65, dimension:'64  x  2,0', di:60},
  {dn:80, dimension:'76,1  x  2,0', di:72.1},
  {dn:100, dimension:'108  x  2,5', di:103}
];

const meplaDimensions = [
  {dn:12, dimension:'16  x  2,25', di:11.5},
  {dn:15, dimension:'20  x  2,5', di:15},
  {dn:20, dimension:'26  x  3,0', di:20},
  {dn:25, dimension:'32  x  3,0', di:26},
  {dn:32, dimension:'40  x  3,5', di:33},
  {dn:40, dimension:'50  x  4,0', di:42},
  {dn:50, dimension:'63  x  4,5', di:54},
  {dn:65, dimension:'75  x  4,7', di:65.6}
];
...
export const pipeSystems = [
  ...
  { id:'plastic', label:'Kunststoff', normSmall:'Systemabhaengig', normLarge:'Systemabhaengig', roughness:0.007, maxDn:160, dimensions: steelDimensions.filter(p => p.dn <= 150) }
];
...
function rating(pressureLoss, maxPressurePam) {
  ...
  if (ratio < .75) return { key:'green', label:'gruen' };
...
export function recommendPipe({ massFlowKgh, volumeFlowM3h, flowValue, flowUnit = 'kg/h', maxPressurePam = 100, systemId = 'steel', density = 998 }) {
  ...
  if (combined && flowUnit === 'm3/h') volume = combined;
