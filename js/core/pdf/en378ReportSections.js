...
const bool = value => value === true || value === 'yes' ? 'ja' : value === false || value === 'no' ? 'nein' : '—';
...
function status(value) {
  return STATUS[value] || text(value);
}

function label(value) {
  return OPTIONS[value] || text(value);
}

function checkLabel(value) {
  return CHECKS[value] || FIELDS[value] || text(value);
}
...
function sourceLabel(source = {}) {
  const part = source.sourcePart || '';
  const section = source.sourceSection || '';
...
function importedSystemSection(dto) {
  ...
  return {
    ...
    rows: [
      ...
      num('Füllmenge', input.chargeKg, 'mass', 'kg'),
...
function chargeLimitSection(dto) {
  ...
  const rows = [
    ...
    num('Maximal zulässige Füllmenge', chargeLimit.maximumAllowedChargeKg, 'mass', 'kg')
  ];
  array(chargeLimit.checks).forEach(check => {
    rows.push(row(checkLabel(check.id), status(check.status), sourceLabel(check.source)));
    if (check.maximumChargeKg != null) rows.push(num(`${checkLabel(check.id)} – Grenzwert`, check.maximumChargeKg, 'mass', 'kg'));
  });
...
function safetyComponentsSection(dto) {
  ...
  array(installationSafety.requirements).forEach(item => {
    rows.push(row(item.title || checkLabel(item.id), `${status(item.status)}: ${item.measure || item.requirement || 'prüfen'}`, sourceLabel(item.source)));
  });
  return { title: '5. Sicherheitskomponenten nach EN 378-3', rows, singleColumn: true };
}
...
function guidanceSection(dto) {
  ...
  return { title: '6. Planer-Leitfaden', rows, singleColumn: true };
}
...
function confirmedSection(dto) {
  ...
  return {
    title: '7. Bestätigte Prüfpunkte',
    rows: rows.length ? rows : [row('Status', 'Keine bestätigten Prüfpunkte ausgewiesen.')],
    singleColumn: true
  };
}
...
function sourcesSection(dto) {
  ...
  return {
    title: '8. Datenstand und Berichtserstellung',
    rows: [
