...
import { card, field, selectField, segmented, renderModuleShell, stack, grid, inlineStats, mainResult, resultCard } from '../../core/renderer.js';
...
function warnList(items){
  ...
  return `<div class="result-list">${items.map(w => `<div class="result-row"><span>Hinweis</span><strong>${w}</strong></div>`).join('')}</div>`;
}

function view(s){
  const r = calculate(s);
  const main = s.holdingType === 'dynamic'
    ? { label:'Erforderliches Nennvolumen dynamisch', value:fmt(r.vnDynamic,1), unit:'Liter' }
    : { label:'Erforderliches MAG-Nennvolumen', value:fmt(r.vnMag,1), unit:'Liter' };
  const details = [
  ...
  const pressureFields = [
    ...
    field({ id:'staticPressureBar', label:'statischer Druck pₛₜ', value:fmtInput(s.staticPressureBar,2), unit:'bar' }),
  ...
  const inputColumn = stack([
    ...
    card('Druckdaten', grid(pressureFields.join(''), 2), 'purple'),
    s.holdingType === 'mag' ? card('MAG-Optionen', segmented('includeServitec', opts([['false','ohne Servitec'],['true','mit Servitec +5 l']]), s.includeServitec, { accent:'purple' }), 'purple') : card('Dynamisches System', selectField({ id:'dynamicType', label:'Arbeitsbereich AD', value:s.dynamicType, options:opts([['reflexomat','Reflexomat · AD 0,2 bar'],['variomat','Variomat · AD 0,4 bar']]) }), 'purple')
  ].join(''));
  ...
  const resultColumn = stack([
    ...
    card('Formeln / Plausibilität', stack([
      `<div class="formula">p₀ = pₛₜ + pD ${s.connectionType === 'pressure' ? '+ Δpₚ' : '+ 0,2 bar'} · pe = pSV − ASV</div>`,
      `<div class="formula">Ve = VA × n / 100 · VV = max(0,5 % × VA; 3 l)</div>`,
      s.holdingType === 'mag'
        ? `<div class="formula">Vn = (Ve + VV${s.includeServitec === 'true' ? ' + 5 l' : ''}) × (pe + 1) / (pe − p₀)</div>`
        : `<div class="formula">Vn ≥ 1,1 × (Ve + VV) · pe ≥ p₀ + 0,3 bar + AD</div>`,
      warnList(r.warnings)
    ].join('')), 'purple')
  ].join(''));
