...
function warnList(items){
  if(!items.length) return '<div class="empty-state empty-state--compact ph-note">Keine Plausibilitaetswarnungen.</div>';
  ...
  return `<div class="ph-saved-list">${items.map(item => {
    ...
    return `<article class="ph-saved-item line-section-card is-collapsed ${state.get().activePlantId === item.id ? 'is-active' : ''}" data-line-card data-ph-select="${esc(item.id)}">
      <div class="line-section-card__head">
        ...
        <button type="button" class="line-section-card__toggle" data-line-toggle aria-expanded="false" aria-label="Gespeicherte Anlage aufklappen"><span>v</span></button>
        <button type="button" class="line-section-card__delete" data-ph-delete="${esc(item.id)}" aria-label="Anlage loeschen"> x </button>
      </div>
      <div class="line-section-card__body">${resultRows([
        { label:'Station / Gefaess', value:res.productLabel || '-' },
        ...
        { label:'Mindestbetriebsdruck p0', value:res.p0 !== undefined ? fmt(res.p0,2) : '-', unit:res.p0 !== undefined ? 'bar' : '' },
        { label:'Enddruck pe', value:res.pe !== undefined ? fmt(res.pe,2) : '-', unit:res.pe !== undefined ? 'bar' : '' }
      ])}</div>
    </article>`;
  }).join('')}</div>`;
}
...
function explain(s){
  if(s.holdingType === 'mag'){
    return `MAG statisch: Membran-Druckausdehnungsgefaess mit Gaspolster. ${s.includeServitec === 'true' ? 'Mit Servitec wird nach Reflex-Formblatt ein Zusatzvolumen von 5 l beruecksichtigt.' : 'Ohne Servitec wird nur Ausdehnungsvolumen und Wasservorlage angesetzt.'}`;
  }
  return s.dynamicType === 'variomat'
    ? 'Variomat: pumpengesteuerte Druckhaltestation. Das Nennvolumen wird dynamisch mit 1,1  x  (Ve + VV) angesetzt; Arbeitsbereich AD 0,4 bar.'
    : 'Reflexomat: kompressorgesteuerte Druckhaltestation. Das Nennvolumen wird dynamisch mit 1,1  x  (Ve + VV) angesetzt; Arbeitsbereich AD 0,2 bar.';
}
...
function view(s){
  ...
  const details = [
    { label: s.holdingType === 'dynamic' ? 'Station / Gefaess' : 'Auswahl MAG', value:r.productLabel },
    { label:'Naechstes Normvolumen', value: r.selectedVolume > 0 ? fmt(r.selectedStandardVolume,0) : '-', unit:'Liter' },
    { label:'Mindestbetriebsdruck p0', value:fmt(r.p0,2), unit:'bar' },
    { label:'Anfangsdruck pa min.', value:fmt(r.paMin,2), unit:'bar' },
    { label:'Enddruck pe', value:fmt(r.pe,2), unit:'bar' }
  ];

  const basis = card('Berechnungsart', stack([
    segmented('systemType', opts([['heating','Heizwasser'],['cooling','Kuehlwasser']]), s.systemType, { accent:'purple' }),
  ...
  const volumeFields = [
    selectField({ id:'waterContentMode', label:'Anlagenvolumen', value:s.waterContentMode, options:opts([['known','bekannt eingeben'],['estimated','ueber Leistung schaetzen']]) }),
    s.waterContentMode === 'estimated'
      ? field({ id:'heatPowerKw', label:'Gesamtleistung Q', value:fmtInput(s.heatPowerKw,1), unit:'kW' })
      : field({ id:'systemVolumeL', label:'Anlagenvolumen Va', value:fmtInput(s.systemVolumeL,1), unit:'Liter' }),
    s.waterContentMode === 'estimated'
      ? field({ id:'specificWaterContent', label:'spez. Wasserinhalt va', value:fmtInput(s.specificWaterContent,1), unit:'l/kW' })
      : field({ id:'additionalVolumeL', label:'Zusatzvolumen', value:fmtInput(s.additionalVolumeL,1), unit:'Liter' })
  ];

  const pressureFields = [
    field({ id:'staticHeightM', label:'statische Hoehe H', value:fmtInput(s.staticHeightM,1), unit:'m' }),
    field({ id:'staticPressureBar', label:'statischer Druck pst manuell', value:fmtInput(s.staticPressureBar,2), unit:'bar' }),
    s.connectionType === 'pressure' ? field({ id:'pumpPressureBar', label:'Pumpendifferenzdruck Deltapp', value:fmtInput(s.pumpPressureBar,2), unit:'bar' }) : '',
  ...
  const inputColumn = stack([
    ...
    card('Temperaturen / Stoffwerte', grid([
      ...
      field({ id:'tMinC', label:'tiefste Systemtemperatur', value:fmtInput(s.tMinC,1), unit:' GradC' }),
      field({ id:'tMaxC', label:'hoechste Temperatur tTR/tmax', value:fmtInput(s.tMaxC,1), unit:' GradC' })
    ].join(''), 2), 'purple'),
    card('Druckdaten', `${grid(pressureFields.join(''), 2)}<p class="ph-help">Ist die statische Hoehe eingetragen, wird pst automatisch mit H/10 berechnet. Der manuelle pst-Wert gilt nur ohne Hoehenangabe.</p>`, 'purple'),
    savedPlantsCard(s),
    s.holdingType === 'mag' ? card('MAG-Optionen', `${segmented('includeServitec', opts([['false','ohne Servitec'],['true','mit Servitec +5 l']]), s.includeServitec, { accent:'purple' })}<p class="ph-help">Servitec steht fuer Entgasung/Nachspeisung. Bei "mit Servitec" wird das Zusatzvolumen des Entgasungsrohres beruecksichtigt.</p>`, 'purple') : card('Dynamisches System', `${selectField({ id:'dynamicType', label:'Druckhaltestation', value:s.dynamicType, options:opts([['reflexomat','Reflexomat  -  kompressorgesteuert  -  AD 0,2 bar'],['variomat','Variomat  -  pumpengesteuert  -  AD 0,4 bar']]) })}<p class="ph-help">Die Auswahl bestimmt Arbeitsbereich AD und die Ergebnisbezeichnung der Station.</p>`, 'purple')
  ].join(''));
  ...
  const resultColumn = stack([
    ...
    resultCard('Zwischenergebnisse', [
      ...
      { label:'statischer Druck pst verwendet', value:fmt(r.staticPressure,2), unit:'bar' },
      { label:'Ausdehnungsvolumen Ve', value:fmt(r.ve,1), unit:'Liter' },
      { label:'Wasservorlage VV', value:fmt(r.vv,1), unit:'Liter' },
      { label:'Schliessdruckdifferenz ASV', value:fmt(r.asv,2), unit:'bar' },
      { label:'Volumenfaktor MAG', value:fmt(r.factor,2) }
    ], 'purple'),
    card('Formeln / Plausibilitaet', stack([
      `<div class="formula ph-formula">p0 = pst + pD ${s.connectionType === 'pressure' ? '+ Deltapp' : '+ 0,2 bar'}  -  pe = pSV  ASV</div>`,
      `<div class="formula ph-formula">Ve = VA  x  n / 100  -  VV = max(0,5 %  x  VA; 3 l)</div>`,
      s.holdingType === 'mag'
        ? `<div class="formula ph-formula">Vn = (Ve + VV${s.includeServitec === 'true' ? ' + 5 l' : ''})  x  (pe + 1) / (pe  p0)</div>`
        : `<div class="formula ph-formula">Vn >= 1,1  x  (Ve + VV)  -  pe >= p0 + 0,3 bar + AD</div>`,
      warnList(r.warnings)
    ].join('')), 'purple')
  ].join(''));
