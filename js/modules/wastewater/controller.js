...
function fixtureSummary(item = {}) {
  return [
    ...
    `DU-Wert ${displayNumber(item.du, 2)} DU/Stk.`,
    `Summe DU ${displayNumber(item.totalDu, 2)} DU`,
...
function wastewaterReportRows(current = {}, result = {}) {
  ...
  if (current.lineType === 'stack') baseRows.push(['Abzweigart Fallleitung', BRANCH_TYPE_LABELS[current.branchType] || 'mit Innenradius', '']);
  if (['branch-vented', 'collector', 'ground-inside', 'ground-outside'].includes(current.lineType)) baseRows.push(['Füllungsgrad', FILL_RATIO_LABELS[String(current.fillRatio || '0.5')] || String(current.fillRatio || '—'), '']);
  ...
  return [
    ...
    ...fixtureDisplayRows(result.fixtures || []),
