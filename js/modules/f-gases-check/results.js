...
function serviceScenarioRows(details = {}) {
  return (details.originScenarios || []).map(item => ({ label: `Servicekältemittel: ${originLabel(item.refrigerantOrigin)}`, value: serviceLabel(item) }));
}
...
export function buildFGasesResultModel(state = {}, result = {}) {
  ...
  return {
    ...
    groups: [
      ...
      { title: 'Wartung und Instandhaltung', rows: [{ label: 'Gesamtbewertung', value: serviceLabel(service) }, ...serviceScenarioRows(service), { label: 'Rechtsgrundlage Verwendung', value: 'Art. 13 der Verordnung (EU) 2024/573' }] },
      ...
      { title: 'Dokumentationspflichten', rows: obligationRows(docs.obligations || [], state).length ? obligationRows(docs.obligations || [], state) : [{ label: 'Status', value: formatFGasesStatus(docs.status) }] },
      { title: 'Betreiberpflichten', rows: obligationRows(duties.obligations || [], state).length ? obligationRows(duties.obligations || [], state) : [{ label: 'Status', value: formatFGasesStatus(duties.status) }] }
    ],
    notices
  };
}
