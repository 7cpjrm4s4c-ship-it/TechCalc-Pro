...
const modeLabel = value => ({ roof:'Dachflaeche', property:'Grundstuecksflaeche' }[value] || value);
const drainLabel = mode => mode === 'property' ? 'Hoftoepfe' : 'Dacheinlaeufe';
const drainCapacityLabel = mode => mode === 'property' ? 'Abflussvermoegen Hoftopf' : 'Abflussvermoegen Dacheinlauf';
const rainLabel = mode => mode === 'property' ? 'Regenspende r(5,2)' : 'Regenspende r(5,5)';
const drainOptions = roofDrainTable.map(item => ({ value:item.dn, label:`${item.dn}  -  ${String(item.capacity).replace('.', ',')} l/s  -  ${item.head} mm Anstauhoehe` }));
const emergencyTypeOptions = opts([['rect','Rechteckiger Notueberlauf'],['round','Runder Notueberlauf'],['manual','Herstellerwert / freie Eingabe']]);
  ...
  return `<div class="ph-saved-list">${items.map(item => {
    ...
    return `<article class="line-section-card saved-record-card is-collapsed ${active ? 'is-active' : ''}" data-line-card data-rainwater-select="${esc(item.id)}">
      <div class="line-section-card__head saved-record-card__head">
        ...
        <button type="button" class="line-section-card__toggle saved-record-card__toggle" data-line-toggle aria-expanded="false" aria-label="Gespeicherte Berechnung aufklappen"><span>v</span></button>
        <button type="button" class="line-section-card__delete saved-record-card__delete" data-rainwater-delete="${esc(item.id)}" aria-label="Berechnung loeschen"> x </button>
      </div>
      <div class="line-section-card__body">${resultRows([
        { label:'Entwaesserungsmenge', value:fmt(r.qr || 0,2), unit:'l/s' },
...
function surfacesTable(r, s) {
  if (!r.surfaces.length) return '<div class="empty-state empty-state--compact">Noch keine Regenflaechen hinzugefuegt.</div>';
  ...
  return `<div class="dw-consumer-list wastewater-fixture-list rainwater-surface-list">${r.surfaces.map(item => {
    ...
    return `<div class="dw-consumer-row wastewater-fixture-row rainwater-surface-row ${active ? 'is-active' : ''}" data-surface-select="${esc(item.id)}">
      <div><strong>${esc(item.name)}</strong><span>${fmt(item.area,1)} m2  -  Cs ${fmt(item.cs,2)}  -  Qr ${fmt(item.qr,2)} l/s  -  FL ${item.stackSelection?.dn || '-'}</span></div>
      <button type="button" data-surface-delete="${esc(item.id)}" aria-label="Regenflaeche entfernen"> x </button>
    </div>`;
  }).join('')}</div>`;
}
function modeCard(s) {
  const mode = s.surfaceMode || s.calculationType || 'roof';
  return card('Berechnungsbereich', segmented('surfaceMode', opts([['roof','Dachflaeche'],['property','Grundstuecksflaeche']]), mode, { accent:'green' }), 'green');
}
function rainInputBlock(s) {
  ...
  return stack([
    ...
    `<a class="action-button action-button--secondary rainwater-kostra-link" href="${esc(KOSTRA_URL)}" target="_blank" rel="noopener">KOSTRA / OpenKo Daten oeffnen</a>`
  ].join(''));
}
function dimensionInputBlock(s) {
  ...
  const fields = [
    ...
    field({ id:'drainHead', label:'Anstauhoehe manuell', value:fmtInput(s.drainHead,0), unit:'mm' })
  ];
...
function emergencyInputBlock(s) {
  const mode = s.surfaceMode || s.calculationType || 'roof';
  if (mode !== 'roof') return '<div class="empty-state empty-state--compact">Notentwaesserung wird nur fuer Dachflaechen vorbemessen.</div>';
  const type = s.emergencyType || 'rect';
  const fields = [
    selectField({ id:'emergencyType', label:'Art Notentwaesserung', value:type, options:emergencyTypeOptions }),
    field({ id:'emergencyHead', label:'Druckhoehe / Anstauhoehe', value:fmtInput(s.emergencyHead || '35',0), unit:'mm' })
  ];
  if (type === 'rect') fields.push(field({ id:'emergencyWidth', label:'Ueberlaufbreite je Notueberlauf Lw', value:fmtInput(s.emergencyWidth || '300',0), unit:'mm' }));
...
function surfaceInputBlock(s, r) {
  ...
  return stack([
    grid([
      field({ id:'areaName', label:'Bezeichnung', value:s.areaName || '', placeholder:'z. B. Dachflaeche Nord', inputmode:'text' }),
      selectField({ id:'areaType', label:'Flaechenart', value:areaType, options:areaOptionsForMode(mode) }),
      field({ id:'areaSize', label:'Flaeche A', value:fmtInput(s.areaSize,1), unit:'m2' }),
    ...
    `<div class="tc-save-actions"><button type="button" class="action-button action-button--secondary" data-surface-add>Flaeche hinzufuegen</button><button type="button" class="action-button" data-surface-update ${s.activeSurfaceId ? '' : 'disabled'}>Aktualisieren</button></div>`
  ].join(''));
}
function saveCard(s) {
  return card('Berechnung speichern', stack([
    field({ id:'name', label:'Bezeichnung', value:s.name || '', placeholder:'z. B. Dachflaeche Verwaltung', inputmode:'text' }),
...
function inputCards(s, r) {
  return stack([
    ...
    card('Dacheinlaeufe / Hoftoepfe', dimensionInputBlock(s), 'green'),
    card('Notentwaesserung', emergencyInputBlock(s), 'green'),
    card('Regenflaechen', surfaceInputBlock(s, r), 'green')
  ].join(''));
}
...
function surfaceDimensionCards(r, s) {
  if (!r.surfaces.length) return '<div class="empty-state empty-state--compact">Keine Einzelflaechen berechnet.</div>';
  const activeId = String(s.activeSurfaceId || '');
  return `<div class="ph-saved-list rainwater-result-list">${r.surfaces.map((item) => {
    ...
    const mainRows = [
      ...
      { label:'Flaeche', value:fmt(item.area,1), unit:'m2' },
      { label:'Abflussbeiwert Cs', value:fmt(item.cs,2) },
      { label:'Entwaesserungsmenge Qr', value:fmt(item.qr,2), unit:'l/s' },
      { label:drainLabel(mode), value:item.requiredDrains, unit:'Stk.' },
      { label:'gewaehlte Ablaufdimension', value:item.drainSize || '-' },
      { label:'Ablaufleistung je Ablauf', value:fmt(item.drainCapacity,1), unit:'l/s' },
      { label:'Anstauhoehe Ablauf', value:item.drainHead || '-', unit:item.drainHead ? 'mm' : '' }
    ];
    ...
    const emergencyRows = isRoof ? [
      ...
      { label:'Art Notentwaesserung', value:item.emergency?.type === 'round' ? 'Rund' : item.emergency?.type === 'manual' ? 'Herstellerwert' : 'Rechteckig' },
      { label:'Notueberlaeufe', value:item.emergency?.requiredCount || 0, unit:'Stk.' },
      { label:'erforderliche Ueberlaufbreite je Notueberlauf', value:item.emergency?.rectWidthPerOverflow ? fmt(item.emergency.rectWidthPerOverflow,0) : '-', unit:item.emergency?.rectWidthPerOverflow ? 'mm' : '' },
      { label:'Notueberlauf-DN', value:item.emergency?.manufacturerDn || '-' },
      { label:'gewaehlte Ueberlaufleistung je Notueberlauf', value:item.emergency?.capacity ? fmt(item.emergency.capacity,2) : '-', unit:item.emergency?.capacity ? 'l/s' : '' }
    ] : [];
    return `<article class="line-section-card ${active ? 'is-active' : ''} is-collapsed" data-line-card data-surface-result-select="${esc(item.id)}">
      <div class="line-section-card__head">
        <div class="line-section-card__title"><strong>${esc(item.name)}</strong><small>${fmt(item.area,1)} m2  -  Qr ${fmt(item.qr,2)} l/s  -  ${drainLabel(mode)} ${item.requiredDrains} Stk.${isRoof ? `  -  FL ${item.stackSelection?.dn || '-'}` : ''}</small></div>
        <button type="button" class="line-section-card__toggle" data-line-toggle aria-expanded="false" aria-label="Flaechendimensionierung aufklappen"><span>v</span></button>
      </div>
      <div class="line-section-card__body">
        <div class="rainwater-result-group"><h4>Hauptentwaesserung</h4>${resultRows(mainRows)}</div>
        ${isRoof ? `<div class="rainwater-result-group"><h4>Notentwaesserung</h4>${resultRows(emergencyRows)}</div>` : ''}
        <div class="ph-formula ph-formula--small">Qr = r  x  C  x  A / 10000  -  Anzahl Ablaeufe = Qr / Ablaufleistung  -  Q je Fallleitung = Qr / Anzahl Fallleitungen</div>
      </div>
    </article>`;
  }).join('')}</div>`;
}
function resultCards(s, r) {
  ...
  const isRoof = mode === 'roof';
  const secondary = [
    { label:'Entwaesserungsmenge', value:fmt(r.qr,2), unit:'l/s' },
    { label:'Ablaufdimension', value:r.drainSize || '-' },
    { label:'Ablaeufe', value:r.requiredDrains, unit:'Stk.' },
    { label:'markierte Flaeche', value:r.selectedSurface?.name || '-' }
  ];
  ...
  return stack([
    ...
    card('Flaechen / Berechnung', surfaceDimensionCards(r, s), 'green'),
    card('Normhinweise / Plausibilitaet', warningList(r.warnings, s), 'green')
  ].join(''));
}
function view(s) {
  const r = calculate(s);
